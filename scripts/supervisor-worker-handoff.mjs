...
const WRITABLE_TASK_STATES = new Set([
  "TASK_CREATED",
  "TASK_DISPATCHED",
  "WORKER_PR_OPENED",
  "CHECKS_PENDING",
...
function collectWritableTaskIds(comments, excludeTaskId = "") {
  ...
  for (const comment of comments) {
    ...
    if (!previous || createdAt > previous.created_at || (createdAt === previous.created_at && id > previous.id)) {
...
export function evaluateDispatchContext(context) {
  ...
  for (const rawPr of prs) {
    ...
    if (isWorkerPr) {
      ...
      const metadata = parseTaskMetadata(pr.body);
      if (metadata.taskId && metadata.taskId !== packet.task_id) activeWorkerIds.add(metadata.taskId);
      if (!metadata.taskId) activeWorkerIds.add(`pr:${pr.number}`);
  ...
  if (activeWorkerIds.size >= 2) {
    return blocked(
      `global writable Worker limit is already two (${activeWorkerIds.size}/2); refusing a third Worker`,
      packet,
      "writable_worker_limit",
    );
  }
