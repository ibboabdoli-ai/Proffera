...
- CodeRabbit remains the primary provider for risk-routed final PR review. Medium-risk PRs may use the configured availability fallback after machine-observed CodeRabbit failure or the bounded response window. High-risk PRs remain CodeRabbit-only during the normal review window, but exact-head Codex fallback may become eligible only after machine-observed CodeRabbit unavailability following the exact-head request or after the bounded high-risk provider timeout.
...
- Workflow, auth/RBAC, tenant/workspace, database/migration, payment, privacy/Directory, package/lockfile, environment/secret, and deployment-sensitive paths remain CodeRabbit-primary and CodeRabbit-only during the normal review window. After the exact-head CodeRabbit request, machine-observed provider unavailability or the bounded high-risk timeout may enable exact-head Codex fallback; if neither provider produces an acceptable exact-head result, CI must fail closed.
