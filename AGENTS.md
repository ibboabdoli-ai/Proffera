...
- A current-head CodeRabbit `CHANGES_REQUESTED` decision is always blocking. Codex can never override it; only a later current-head CodeRabbit `APPROVED` decision or a fresh authorized human policy decision may clear that state.
