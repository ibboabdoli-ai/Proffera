...
- **CodeRabbit** — keep the existing final risk-routed PR review as the authoritative external review gate. For Class 3/4 work, or work that would match the repository's sensitive/large AI-review routing, add one local pre-push CodeRabbit CLI pass **after targeted validation and the internal Red Team, but before the first implementation push**, when the CLI is already installed and authenticated. Use `cr review --agent --base main` so committed, staged, and tracked local changes are reviewed together. Treat findings as hypotheses: fix only verified critical/major issues, rerun the affected checks, and allow at most two CLI review passes for the same bounded task. Do not run CLI review after every commit. If the CLI is unavailable or unauthenticated, do not install software, add credentials, or expand the task just to enable it; report `CodeRabbit CLI: unavailable` and continue to the required current-head PR review gate.
