...
2. The generic handoff has two trusted entry points. The event router invokes it by `workflow_dispatch` with an immutable source comment ID, and the handoff re-fetches that owner-authored #548 comment before admission. The planner invokes it by `workflow_call` with `planner_packet_b64`, the packet digest (`planner_packet_sha256`), planner run ID, planner head SHA, and planner workflow ref; those values are verified before admission. Direct comment fan-out is no longer part of the handoff workflow.
3. Deterministic admission enforces a maximum of two concurrently writable Phase-1 Workers. Admission uses both durable task-state reservations and trusted open Task-Packet PRs so the second Worker can run in parallel while a third waits. Legacy PRs remain overlap-checked but do not silently consume Phase-1 capacity unless they carry the trusted Task Packet marker.
4. `supervisor-planner.yml` runs only when both `worker-dispatch-enabled` and `supervisor-autopilot-enabled` are present on #548. Codex may propose one bounded task, but repository code re-parses and re-admits the proposal against live main, open PRs, durable task states, scope ownership and the two-Worker ceiling before the validated packet is passed internally to `supervisor-worker-handoff.yml`. The planner does not publish a Task Packet comment to #548.
5. `supervisor-review-repair.yml` batches review bursts for Phase-1 PRs, verifies findings against the exact current head, validates the actual repair diff against the embedded Task Packet, runs full validation, and publishes at most one same-branch repair commit per batch. Two consecutive automatic repair commits stop further automatic repair to prevent loops.

The planner never grants merge authority and cannot select control-plane, workflow, secret/configuration, package/lockfile, or migration/schema work. Control-plane maintenance remains a separately reviewed branch. Human exact-head merge authorization and all Production mutation restrictions remain unchanged.
