...
SCB enrichment projects a company municipality into the main Directory profile only when the profile municipality is blank, records SCB field provenance, and never overwrites an existing municipality. The relational service catalog includes the runtime `frisor` mapping for primary SNI 96.210 and backfills SNI-owned profile/service relations. Neither repair changes publication status. Geographic coordinates remain a separate controlled Lantmäteriet geocoding pilot; absence of a verified coordinate is not treated as an SCB-sync failure and broad geocoding must not be enabled until the upstream PROD lookup path produces verified references.
