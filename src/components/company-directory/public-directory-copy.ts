...
export const directoryCopy = {
  sv: {
    ...
    addressNotice: "Ortssökning använder företagets verksamhetsplats när den kan fastställas. Bekräftat serviceområde visas separat.",
    nearbyNotice: (radius: number) => `Nära mig visar företag med verifierad position inom ${radius} km. Bekräftat serviceområde visas separat. När Lantmäteriets positionsdata används anges källan som Lantmäteriet – Belägenhetsadress Direkt; avståndet beräknas av Proffera (egen bearbetning).`,
    ...
    results: "Företag som matchar", companyCount: (count: number) => `${count} ${count === 1 ? "företag matchar" : "företag matchar"} din sökning`, nearest: (radius: number) => `Verifierad position · inom ${radius} km`, withinRadius: (radius: number) => `Verifierad position · inom ${radius} km`,
    ...
    publishedOnly: "Verifierade företagsprofiler", officialData: "Officiella företagsdata", verifiedDetails: "Företagsuppgifter verifierade", illustration: "Illustrationsbild", away: (km: number) => `${km.toFixed(1)} km bort`,
    registeredIn: (location: string) => `Verksamhetsplats: ${location}`,
  ...
  en: {
    ...
    addressNotice: "Location search uses the business's operating location when it can be established. Confirmed service area is shown separately.",
    nearbyNotice: (radius: number) => `Near me shows businesses with a verified position within ${radius} km. Confirmed service area is shown separately. Where Lantmäteriet position data is used, the source is Lantmäteriet – Belägenhetsadress Direkt; distance is calculated by Proffera (processed by Proffera).`,
    ...
    results: "Matching businesses", companyCount: (count: number) => `${count} ${count === 1 ? "business matches" : "businesses match"} your search`, nearest: (radius: number) => `Verified position · within ${radius} km`, withinRadius: (radius: number) => `Verified position · within ${radius} km`,
    ...
    publishedOnly: "Verified business profiles", officialData: "Official company data", verifiedDetails: "Company details verified", illustration: "Illustration", away: (km: number) => `${km.toFixed(1)} km away`,
    registeredIn: (location: string) => `Business location: ${location}`,
