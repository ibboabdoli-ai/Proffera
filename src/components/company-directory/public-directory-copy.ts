...
export const directoryCopy = {
  sv: {
    ...
    results: "Företag som matchar", companyCount: (count: number) => `${count} ${count === 1 ? "företag matchar" : "företag matchar"} din sökning`, nearest: (radius: number) => `Närmaste först · ${radius} km`,
    range: (from: number, to: number, total: number) => `Visar ${from}–${to} av ${total} företag`, previous: "Föregående", next: "Nästa", pageLabel: (page: number) => `Sida ${page}`,
  ...
  en: {
    ...
    results: "Matching businesses", companyCount: (count: number) => `${count} ${count === 1 ? "business matches" : "businesses match"} your search`, nearest: (radius: number) => `Nearest first · ${radius} km`,
    range: (from: number, to: number, total: number) => `Showing ${from}–${to} of ${total} businesses`, previous: "Previous", next: "Next", pageLabel: (page: number) => `Page ${page}`,
