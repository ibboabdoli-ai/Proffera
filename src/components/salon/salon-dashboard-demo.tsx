  ...
  return (
    <div className="grid gap-8">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#dfe5dd]"><p className="text-sm font-semibold text-[#5b665f]">Väntande bokningar</p><p className="mt-3 text-3xl font-bold text-[#17452f]">{pendingCount}</p><p className="mt-2 text-sm text-[#5b665f]">Behöver godkännas</p></article>
        <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#dfe5dd]"><p className="text-sm font-semibold text-[#5b665f]">Bekräftade tider</p><p className="mt-3 text-3xl font-bold text-[#17452f]">{confirmedCount}</p><p className="mt-2 text-sm text-[#5b665f]">Kommande bokningar</p></article>
        <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#dfe5dd]"><p className="text-sm font-semibold text-[#5b665f]">Aktiva tjänster</p><p className="mt-3 text-3xl font-bold text-[#17452f]">{salonServices.length}</p><p className="mt-2 text-sm text-[#5b665f]">Synliga på bokningssidan</p></article>
        <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#dfe5dd]"><p className="text-sm font-semibold text-[#5b665f]">Personal</p><p className="mt-3 text-3xl font-bold text-[#17452f]">1</p><p className="mt-2 text-sm text-[#5b665f]">Elias aktiv</p></article>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#dfe5dd]">
          <div className="flex items-center justify-between gap-4">
            ...
            <button className="inline-flex items-center gap-2 rounded-full bg-[#17452f] px-4 py-2 text-sm font-semibold text-white" type="button"><Plus className="h-4 w-4" aria-hidden="true" /> Ny bokning</button>
          </div>

          {bookings.length > 0 ? <p className="mt-4 rounded-2xl bg-[#e7f1eb] px-4 py-3 text-sm font-bold text-[#17452f]">Live demo: bokningar från formuläret visas här i samma webbläsare.</p> : <p className="mt-4 rounded-2xl bg-[#f7f7f4] px-4 py-3 text-sm font-bold text-[#5b665f]">Skapa en testbokning på demosidan för att se den här.</p>}
            ...
            {visibleBookings.map((booking) => (
              <article key={booking.id} className="rounded-2xl border border-[#dfe5dd] bg-[#fbfbf8] p-4">
                <div className="flex flex-col justify-between gap-3 md:flex-row md:items-start">
                  <div>
                    <p className="text-lg font-bold text-[#17201a]">👤 {booking.customer}</p>
                    <div className="mt-3 grid gap-2 text-sm text-[#5b665f]">
                      <p>✂ {booking.service}</p>
                      <p>📅 {booking.date}</p>
                      <p>🕐 {booking.time}</p>
                      <p>💰 {booking.price}</p>
                  ...
                  <span className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${statusClasses[booking.statusTone]}`}>{booking.status}</span>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <a className="rounded-full border border-[#dfe5dd] bg-white px-4 py-2 text-xs font-bold text-[#344139]" href={`tel:${booking.phone}`}>Ring kund</a>
                  <a className="rounded-full border border-[#dfe5dd] bg-white px-4 py-2 text-xs font-bold text-[#344139]" href={`mailto:${booking.email}`}>Mejla</a>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#dfe5dd]">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div><h3 className="text-xl font-bold text-[#17201a]">Tjänster</h3><p className="mt-1 text-sm text-[#5b665f]">Priser och tidsåtgång kan ändras av salongen.</p></div>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {salonServices.slice(0, 6).map((service) => (
            <article key={service.name} className="rounded-2xl border border-[#dfe5dd] bg-[#fbfbf8] p-4"><p className="text-xs font-semibold uppercase tracking-wide text-[#17452f]">{service.category}</p><h4 className="mt-2 font-bold text-[#17201a]">{service.name}</h4><div className="mt-3 flex items-center justify-between text-sm"><span className="text-[#5b665f]">{service.duration}</span><span className="font-bold text-[#17452f]">{service.price}</span></div></article>
          ))}
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {modules.map(({ icon: Icon, title, text }) => (
          <article key={title} className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#dfe5dd]"><Icon className="h-7 w-7 text-[#17452f]" aria-hidden="true" /><h3 className="mt-4 text-lg font-bold text-[#17201a]">{title}</h3><p className="mt-2 text-sm leading-6 text-[#5b665f]">{text}</p></article>
        ))}
      </section>
    </div>
  );
