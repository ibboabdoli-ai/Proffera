...
export const mobileLanguageLinkStyle = {
  color: "#ffffff",
  ...
  return (
    <nav className="grid gap-1.5" aria-label={text.navigation}>
      {dashboardNavigation.map((item) => {
        ...
        const content = <><Icon className="h-[18px] w-[18px] shrink-0" aria-hidden="true" /><span className="flex-1">{label}</span>{isLocked || isPlanned ? <span className={isActive ? "text-[10px] font-bold uppercase tracking-wide text-[#315b8e]" : "text-[10px] font-bold uppercase tracking-wide text-[#9fb8d8]"}>{isLocked ? text.locked : text.planned}</span> : null}</>;

        if (isLocked) {
          return <div key={item.href} aria-disabled="true" className="flex min-h-11 cursor-not-allowed items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-[#9eb0c7] opacity-80" title={text.lockedTitle}>{content}</div>;
        }

        return <NavigationPrefetchLink key={item.href} href={localizedHref(item.href, locale, searchParams)} onClick={onNavigate} aria-current={isActive ? "page" : undefined} className={["group flex min-h-11 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition", isActive ? "bg-white text-[#0a2e63] shadow-[0_8px_24px_rgba(0,0,0,0.12)]" : "text-[#dce8f7] hover:bg-white/10 hover:text-white"].join(" ")} style={isActive ? undefined : { color: "#e3edf9" }}>{content}</NavigationPrefetchLink>;
      })}
    </nav>
  );
}
...
function Brand({ workspaceName, locale }: { workspaceName: string; locale: DashboardLocale }) {
  return <Link href={locale === "en" ? "/en" : "/"} className="flex items-center gap-3 !text-white" style={{ color: "#ffffff" }}><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#eaf2ff] text-lg font-black text-[#0a2e63] shadow-lg shadow-black/15">P</span><span><span className="block truncate text-lg font-bold tracking-tight">{workspaceName}</span><span className="block text-[10px] font-semibold uppercase tracking-[0.18em] text-[#bcd0e8]">{shellCopy[locale].portal}</span></span></Link>;
}
    ...
    <div className="min-h-screen bg-[#f5f7fa] text-[#11213b]" lang={locale}>
      <div className="grid min-h-screen lg:grid-cols-[264px_minmax(0,1fr)]">
        ...
        <div className="flex min-h-screen flex-col">
          <header className="sticky top-0 z-30 border-b border-[#dce4ee] bg-white/90 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8" style={{ paddingTop: "calc(0.75rem + env(safe-area-inset-top))" }}><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><button ref={mobileMenuTriggerRef} type="button" onClick={() => setIsMobileMenuOpen(true)} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#d6e0ec] bg-white text-[#0a2e63] lg:hidden" aria-label={text.openMenu} aria-expanded={isMobileMenuOpen} aria-controls="dashboard-mobile-menu"><Menu className="h-5 w-5" /></button><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#6a7890]">{workspaceName} · {text.portal}</p><h1 className="truncate text-lg font-bold tracking-tight text-[#11213b] sm:text-xl">{currentPageLabel}</h1></div></div><div className="flex items-center gap-2 sm:gap-3"><Link href={languageHref} className="hidden rounded-xl border border-[#d6e0ec] bg-white px-3 py-2 text-xs font-bold text-[#17452f] sm:inline-flex">{text.language}</Link><div className="hidden items-center gap-2 rounded-full bg-[#edf4ff] px-3 py-2 text-xs font-bold text-[#17452f] sm:flex"><span className="h-2 w-2 rounded-full bg-[#2e8b57]" />{text.activeWorkspace}</div><button type="button" onClick={handleSignOut} disabled={isSigningOut} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-[#d6e0ec] bg-white px-3 text-sm font-semibold text-[#4d5d72] disabled:opacity-70"><LogOut className="h-4 w-4" /><span className="hidden sm:inline">{isSigningOut ? text.signingOut : text.signOut}</span></button></div></div></header>
