    ...
    <div className="min-h-screen bg-[#f5f7fa] text-[#11213b]" lang={locale} style={dashboardTheme}>
      <div className="grid min-h-screen lg:grid-cols-[264px_minmax(0,1fr)]">
        ...
        <div className="flex min-h-screen flex-col">
          <header className="sticky top-0 z-30 border-b border-[#dce4ee] bg-white/90 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8" style={{ paddingTop: "calc(0.75rem + env(safe-area-inset-top))" }}><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><button ref={mobileMenuTriggerRef} type="button" onClick={() => setIsMobileMenuOpen(true)} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#d6e0ec] bg-white text-[#0a2e63] lg:hidden" aria-label={text.openMenu} aria-expanded={isMobileMenuOpen} aria-controls="dashboard-mobile-menu"><Menu className="h-5 w-5" /></button><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#6a7890]">{workspaceName} · {text.portal}</p><h1 className="truncate text-lg font-bold tracking-tight text-[#11213b] sm:text-xl">{currentPageLabel}</h1></div></div><div className="flex items-center gap-2 sm:gap-3"><Link href={languageHref} className="hidden rounded-xl border border-[#d6e0ec] bg-white px-3 py-2 text-xs font-bold text-[#0a2e63] sm:inline-flex">{text.language}</Link><div className="hidden items-center gap-2 rounded-full bg-[#edf4ff] px-3 py-2 text-xs font-bold text-[#0a2e63] sm:flex"><span className="h-2 w-2 rounded-full bg-[#2e8b57]" />{text.activeWorkspace}</div><button type="button" onClick={handleSignOut} disabled={isSigningOut} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-[#d6e0ec] bg-white px-3 text-sm font-semibold text-[#4d5d72] disabled:opacity-70"><LogOut className="h-4 w-4" /><span className="hidden sm:inline">{isSigningOut ? text.signingOut : text.signOut}</span></button></div></div></header>
