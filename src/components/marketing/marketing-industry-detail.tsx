    ...
    <div className="overflow-hidden bg-canvas">
      <section className="border-b border-line bg-surface">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
          <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-ink-muted">
            <Link href="/branscher" className="transition hover:text-brand">Branscher</Link>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
        <div className="grid gap-8 lg:grid-cols-[0.75fr_1.25fr] lg:items-start">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.14em] text-brand">Passar när</p>
            <h2 className="mt-3 text-3xl font-black tracking-[-0.03em] text-ink">{page.fitTitle}</h2>
          </div>
          <div className="grid gap-3">
            {page.fitPoints.map((point) => (
              <div key={point} className="flex gap-3 rounded-card border border-line bg-surface p-5 shadow-card">
                <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-brand" />
                <p className="text-sm leading-7 text-ink-muted">{point}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-black tracking-tight text-ink">Proffera för fler tjänsteföretag</h2>
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          {related.map((item) => (
            <Link key={item.slug} href={`/branscher/${item.slug}`} className="group flex items-center justify-between rounded-card border border-line bg-surface px-5 py-4 font-bold text-ink transition hover:border-line-strong hover:text-brand">
              {item.navLabel}
              <ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" />
            </Link>
          ))}
        ...
        <div className="mt-12 flex flex-col items-start justify-between gap-6 rounded-panel border border-line bg-brand-tint p-7 sm:flex-row sm:items-center sm:p-9">
          <div>
            <h2 className="text-2xl font-black text-ink">Testa kundflödet med din egen verksamhet</h2>
