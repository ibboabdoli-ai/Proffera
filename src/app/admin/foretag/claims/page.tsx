import { ArrowLeft, BadgeCheck, Building2, Mail, ShieldCheck } from "lucide-react";
import Link from "next/link";

import { listCompanyDirectoryClaims } from "@/lib/company-directory-claims-admin";

import { approveDirectoryClaimAction, rejectDirectoryClaimAction } from "./actions";
              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <div className="mt-3 grid gap-2 text-sm text-[#5c675f] sm:grid-cols-2">
                    ...
                    <p className="flex items-center gap-2"><BadgeCheck className="h-4 w-4" /> E-post {row.claimant_email_verified ? "verifierad" : "inte verifierad"}</p>
                  </div>
                ...
                <div className="w-full max-w-md space-y-3">
                  <form action={approveDirectoryClaimAction} className="rounded-2xl border border-[#cfe1d4] bg-[#f3f8f4] p-4">
                    ...
                    <textarea
                      ...
                      minLength={3}
                      maxLength={500}
                      placeholder="Exempel: verifierad firmatecknare / intern kontrollreferens"
                    ...
                    />
                    <button type="submit" disabled={!row.claimant_email_verified} className="mt-3 min-h-11 w-full rounded-xl bg-[#17452f] px-4 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40">
                      Verifiera och skapa workspace
                    </button>
                  </form>
