...
export default async function AdminBillingPage({
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8 px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section className="rounded-2xl border border-blue-200 bg-blue-50 p-5 text-sm text-blue-950 shadow-sm">
        <p className="font-bold">Billing safety</p>
        <p className="mt-1">Planbyte, statusändring, cancel och refund är fortsatt read-only. Trial kan endast förlängas för interna trialperioder utan Stripe-subscription. Stripe-hanterade perioder blockeras även om formuläret manipuleras.</p>
      </section>
                ...
                return (
                  <tr key={String(workspace.id)} className={status === "past_due" ? "bg-red-50/40" : ""}>
                    ...
                    <td className="px-4 py-4 text-slate-700">
                      <p>{String(workspace.plan_key ?? "—")}</p>
                      <p className="mt-1 text-xs text-slate-500">{workspace.stripe_bound ? "Stripe-managed" : "Internal"}</p>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4 align-top">
                      <div className="flex flex-col items-start gap-2">
                        ...
                        {workspace.trial_extension_allowed && currentPeriodEnd ? (
                          <details>
                            <summary className="cursor-pointer text-xs font-semibold text-amber-800">Kontrollerad trialförlängning</summary>
                            <TrialExtensionForm
                              workspaceId={String(workspace.id)}
                              workspacePlanId={String(workspace.workspace_plan_id)}
