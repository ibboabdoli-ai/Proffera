...
export async function GET(request: Request) {
  ...
  if (!transport) return NextResponse.json({ ok: false, configured: false, results: [] }, { status: 503, headers: { "Cache-Control": "no-store" } });

  const batchIndex = Number(new URL(request.url).searchParams.get("batch") ?? "") - 1;
  const batch = PILOT_BATCHES[batchIndex];
  if (!batch) return NextResponse.json({ ok: false, error: "invalid_batch" }, { status: 400, headers: { "Cache-Control": "no-store" } });
  ...
  for (const organizationNumber of batch) {
    try {
      ...
      await sleep(1100);
      results.push({ organizationNumber, ok: true, normalized: normalizeScbCompanyRegistryPayload(companyPayload, workplacePayload, organizationNumber) });
    } catch (error) {
      results.push({ organizationNumber, ok: false, error: error instanceof Error ? error.message : "SCB lookup failed" });
    }
  }

  return NextResponse.json({ ok: results.every((result) => result.ok), configured: true, batch: batchIndex + 1, results }, { headers: { "Cache-Control": "no-store, max-age=0", "X-Robots-Tag": "noindex" } });
}
