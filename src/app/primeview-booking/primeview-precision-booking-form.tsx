...
const serviceDescriptions: Record<string, string> = {
  "window cleaning": "Price by property type, real window count, inside/outside, height, access and frequency.",
  "gutter cleaning": "A separate gutter service priced by property type, height, blockage, condition and access.",
  "pressure washing": "A separate pressure-washing service priced by m², dirt / moss and optional treatments.",
  "driveway & patio cleaning": "Pressure washing priced by m², dirt / moss and optional treatments.",
  "gutter + pressure washing": "Best-value package. Both services are priced together with an automatic package saving.",
...
export function PrimeViewPrecisionBookingForm({ action, services, bookingHours, busyBookings, timeZone, initialServiceId = "" }: Props) {
  ...
  const pricingInput = useMemo<PrimeViewPricingInput | null>(() => serviceKey ? ({
    ...
    frequency,
    firstClean,
    propertySize: serviceKey === "window" || serviceKey === "gutter" || serviceKey === "package" ? inferredPropertySize : propertySize,
    heavyBlockage,
    conservatorySize,
    ...
    sealing,
    multiServiceCount: 1,
  }) : null, [access, areaM2, bayWindows, condition, conservatorySize, firstClean, floorCount, frequency, hardAccessWindows, heavyBlockage, heavyDirtMoss, inferredPropertySize, largeWindows, oilTreatment, propertySize, resanding, scope, sealing, serviceKey, solarPanels, standardWindows, weedTreatment, windowAccess, workingHeight]);
  ...
  return (
    <form action={action} className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_350px]">
      ...
      {(serviceKey === "window" || serviceKey === "gutter" || serviceKey === "package") ? <input type="hidden" name="property_size" value={inferredPropertySize ?? ""} /> : null}
      ...
      <div className="grid gap-5">
        ...
        <section className={cardClass}>
          <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#eaf3ff] text-[#1769c2]"><Home className="h-5 w-5" /></div><div><h2 className="text-xl font-black text-[#0b2a4a]">2. Property & price details</h2><p className="mt-1 text-sm text-[#667b91]">Property type, real quantities, height and access all affect the calculation.</p></div></div>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            {serviceKey === "fascia" ? <label className={labelClass}>Property size<select name="property_size" value={propertySize} onChange={(event) => setPropertySize(event.target.value as typeof propertySize)} className={fieldClass}><option>Small property</option><option>Terraced house</option><option>Semi-detached house</option><option>Detached house</option><option>Large property</option></select></label> : null}

            {(serviceKey === "gutter" || serviceKey === "package") ? <div className="sm:col-span-2"><Checkbox name="heavy_blockage" checked={heavyBlockage} onChange={setHeavyBlockage} label="Heavy gutter blockage (+20%)" /></div> : null}
            ...
            {(serviceKey === "patio" || serviceKey === "package") ? <>
              <label className={labelClass}>Pressure-washing area in m²<input name="area_m2" required type="number" min="1" step="0.5" value={areaM2} onChange={(event) => setAreaM2(event.target.value)} placeholder="e.g. 35" className={fieldClass} /></label>
              <div className="grid gap-3 sm:col-span-2 sm:grid-cols-2"><Checkbox name="heavy_dirt_moss" checked={heavyDirtMoss} onChange={setHeavyDirtMoss} label="Heavy dirt / moss (£11/m²)" /><Checkbox name="oil_treatment" checked={oilTreatment} onChange={setOilTreatment} label="Oil / stain treatment (+£3/m²)" /><Checkbox name="weed_treatment" checked={weedTreatment} onChange={setWeedTreatment} label="Weed treatment (+£2/m²)" /><Checkbox name="resanding" checked={resanding} onChange={setResanding} label="Re-sanding (+£4/m²)" /><Checkbox name="sealing" checked={sealing} onChange={setSealing} label="Sealing (Request a Quote)" /></div>
            </> : null}
      ...
      <aside className="grid content-start gap-4 lg:sticky lg:top-6 lg:self-start">
        <section className="overflow-hidden rounded-3xl bg-[#0b2a4a] text-white shadow-[0_18px_55px_rgba(11,42,74,.22)]">
          ...
          <div className="border-t border-white/10 bg-[#09243f] p-5 text-xs leading-5 text-[#c8d9eb]"><p><strong>Window minimums:</strong> Terraced £30 · Semi-detached £39 · Detached £45 · other standard properties £40.</p><p className="mt-2"><strong>Gutter:</strong> from £69, with Terraced £79 · Semi-detached £99 · Detached £129. Large / unusual jobs use Request a Quote.</p><p className="mt-2"><strong>Pressure Washing:</strong> £180 minimum. The Gutter + Pressure Washing package is cheaper than booking the two services separately and shows the exact saving.</p></div>
        </section>
