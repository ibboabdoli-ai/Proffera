...
function StatusBadge({ ready }: { ready: boolean }) {
  return (
    <span className={`rounded-full px-3 py-1 text-xs font-bold ${ready ? "bg-[#eaf8f2] text-[#087754]" : "bg-[#fff7df] text-[#805d14]"}`}>
...
export default async function OnboardingPage({
  ...
  return (
    <div className="grid gap-6">
      <DashboardPageHeader
        eyebrow="Kom igång"
        title="Din bokningssida på under 5 minuter"
        description="Välj din bransch. Proffera skapar starttjänster automatiskt och använder de bokningstider som redan finns i arbetsytan. När de tre stegen är klara kan du dela länken direkt."
        icon={CheckCircle2}
      />
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <article className="rounded-card border border-line bg-surface p-5 shadow-card">
          <div className="flex items-start justify-between gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-control bg-brand-deep text-sm font-black text-white">1</span>
            <StatusBadge ready={launch.servicesReady} />
          </div>
          <h2 className="mt-4 text-lg font-bold text-ink">Bransch och tjänster</h2>
        ...
        <article className="rounded-card border border-line bg-surface p-5 shadow-card">
          <div className="flex items-start justify-between gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-control bg-brand-deep text-sm font-black text-white">2</span>
            <StatusBadge ready={launch.hoursReady} />
          </div>
          <h2 className="mt-4 text-lg font-bold text-ink">Bokningstider</h2>
          <p className="mt-2 text-sm leading-6 text-ink-muted">
            {launch.hoursReady ? "Bokningstider finns redan. Du kan justera dem senare om du vill." : "Lägg in tider då kunder ska kunna boka."}
          </p>
          <a href="/dashboard/installningar" className="mt-3 inline-flex text-sm font-bold text-brand underline underline-offset-4">Justera tider</a>
        </article>

        <article className="rounded-card border border-line bg-surface p-5 shadow-card">
          <div className="flex items-start justify-between gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-control bg-brand-deep text-sm font-black text-white">3</span>
            <StatusBadge ready={launch.bookingReady && launch.servicesReady && launch.hoursReady} />
          </div>
          <h2 className="mt-4 text-lg font-bold text-ink">Dela bokningssidan</h2>
          <p className="mt-2 text-sm leading-6 text-ink-muted">
      ...
      <form action={saveIndustryAndPrepareBooking} className="rounded-card border border-line bg-surface p-5 shadow-card sm:p-6">
        <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
          <label className="grid gap-2 text-sm font-bold text-ink">
            Välj typ av företag
            <select name="industryKey" defaultValue={onboarding.industryKey} className="w-full rounded-control border border-line bg-surface px-4 py-3 font-normal">
            ...
            <span className="text-xs font-normal leading-5 text-ink-muted">Om du redan har tjänster ändras eller raderas de inte.</span>
          </label>
          <button className="inline-flex min-h-12 items-center justify-center rounded-control bg-brand-deep px-6 py-3 text-sm font-bold text-white transition hover:bg-brand-hover">
      ...
      {launch.bookingUrl ? (
        <section className="grid gap-4">
          <div className="rounded-card border border-[#cfe8d6] bg-[#eaf8f2] p-5 shadow-card sm:p-6">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-brand">Redo att ta emot bokningar</p>
            <h2 className="mt-2 text-2xl font-bold text-ink">Din första bokningssida är klar ✅</h2>
            <p className="mt-2 text-sm leading-6 text-ink-muted">Du kan börja dela länken nu. Tema, personal, priser och företagsprofil kan finjusteras efteråt utan att blockera lanseringen.</p>
          </div>
          <BookingLinkCard url={launch.bookingUrl} />
          <a href="/dashboard" className="inline-flex min-h-12 w-fit items-center justify-center rounded-control bg-brand-deep px-6 py-3 text-sm font-bold text-white">Fortsätt till dashboard</a>
        </section>
      ) : (
        <section className="rounded-card border border-line bg-surface-subtle p-5 sm:p-6">
          <h2 className="text-lg font-bold text-ink">Det som återstår</h2>
      ...
      <section className="rounded-[24px] border border-[#e0e5dd] bg-white p-6">
        <h2 className="text-lg font-bold text-ink">Finputs efter lansering</h2>
        <p className="mt-2 text-sm leading-6 text-ink-muted">Det här behöver inte stoppa din första bokningssida.</p>
        <div className="mt-4 flex flex-wrap gap-3">
          <a href="/dashboard/installningar" className="inline-flex min-h-11 items-center rounded-xl border border-line px-5 py-3 text-sm font-bold text-brand">Företagsprofil och tjänster</a>
          <a href="/dashboard/installningar/utseende" className="inline-flex min-h-11 items-center rounded-xl border border-line px-5 py-3 text-sm font-bold text-brand">Tema och utseende</a>
          <a href="/dashboard/personal" className="inline-flex min-h-11 items-center rounded-xl border border-line px-5 py-3 text-sm font-bold text-brand">Lägg till personal</a>
        </div>
      </section>
    </div>
  );
