...
async function saveOnboarding(formData: FormData) {
  ...
  await updateWorkspaceOnboarding({ industryKey, currentStep, completedSteps, isComplete });
  redirect(isComplete ? "/dashboard?onboarding=complete" : "/dashboard/onboarding?saved=1");
}

export default async function OnboardingPage({ searchParams }: { searchParams?: Promise<{ saved?: string }> }) {
  ...
  return <div className="grid gap-6">
    ...
    {params.saved === "1" ? <p className="rounded-xl bg-[#eaf6ed] p-4 text-sm font-bold text-[#17452f]">Onboarding sparades.</p> : null}
    <form action={saveOnboarding} className="grid gap-6">
      <section className="rounded-[24px] border border-[#dfe6df] bg-white p-6"><h2 className="text-xl font-bold">Bransch</h2><p className="mt-2 text-sm text-[#667168]">Branschen används senare för automatiska tjänster, texter, tema och bokningsregler.</p><select name="industryKey" defaultValue={onboarding.industryKey} className="mt-4 w-full rounded-xl border border-[#d7dfd7] px-4 py-3"><option value="salon">Salong och skönhet</option><option value="cleaning">Städning</option><option value="window_cleaning">Fönsterputs</option><option value="consulting">Konsult</option><option value="repair">Reparation och service</option><option value="healthcare">Hälsa</option><option value="restaurant">Restaurang</option><option value="other">Annat</option></select></section>
