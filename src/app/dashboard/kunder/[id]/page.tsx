  ...
  return <div className="grid gap-6">
    ...
    <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
      ...
      <aside className="h-fit rounded-panel border border-line bg-surface p-5 shadow-card sm:p-6 xl:sticky xl:top-24">
        ...
        <div className="mt-5 space-y-3">{events.length === 0 ? <p className="rounded-card border border-line bg-surface-subtle p-4 text-sm text-ink-muted">{isEnglish ? "No events were found." : "Inga händelser hittades."}</p> : events.map((event) => <div key={event.id} className="rounded-card border border-line bg-surface-subtle p-4"><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-brand-soft px-3 py-1 text-xs font-semibold text-brand-deep">{eventTypes[event.type as keyof typeof eventTypes] ?? event.type}</span><span className="text-xs text-ink-muted">{event.createdAt}</span></div><p className="mt-3 font-semibold text-ink">{event.title}</p><p className="mt-2 text-sm leading-6 text-ink-muted">{event.description}</p></div>)}</div>
      </aside>
    </section>
  </div>;
}
