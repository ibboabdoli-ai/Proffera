    ...
    <section className="grid gap-6 lg:grid-cols-[1fr_360px]">
      <div className="grid gap-6">
        ...
        {parsedNotes.photoPaths.length ? <article className="rounded-[24px] border border-[#e0e5dd] bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">{parsedNotes.photoPaths.map((pathname, index) => { const src = `/api/primeview/booking-photo?bookingId=${encodeURIComponent(booking.id)}&pathname=${encodeURIComponent(pathname)}`; return <a key={pathname} href={src} target="_blank" rel="noreferrer" className="overflow-hidden rounded-xl border border-[#e4e9e2] bg-[#f7f9f6]"><Image src={src} alt={`Property photo ${index + 1}`} width={720} height={480} unoptimized className="aspect-[3/2] w-full object-cover" /></a>; })}</div>
        </article> : null}
