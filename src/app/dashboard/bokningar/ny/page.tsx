...
export default async function NewBookingPage({ searchParams }: Props) {
  ...
  const field = "rounded-control border border-line bg-surface px-4 py-3 text-sm font-normal text-ink outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/15";
  const t = en ? labels.en : labels.sv;
  return <div className="grid gap-6">
    <DashboardPageHeader eyebrow={en ? "Bookings" : "Bokningar"} title={en ? "New booking" : "Ny bokning"} description={en ? "Create a booking and connect it directly to a customer. Only owners and administrators can save changes." : "Skapa en ny bokning i Proffera och koppla den direkt till vald kund. Endast ägare och administratörer kan spara ändringar."} icon={CalendarPlus} actions={<Link href={url("/dashboard/bokningar", en)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-control border border-line bg-surface px-4 py-2.5 text-sm font-bold text-brand-deep"><ArrowLeft className="h-4 w-4" />{en ? "Back to bookings" : "Tillbaka till bokningar"}</Link>} />
    {errorKey ? <section className="rounded-card border border-[#f4c7ba] bg-[#fff5f2] p-5 text-sm font-semibold text-danger ring-1 ring-[#f4c7ba]">{(en ? errors.en : errors.sv)[errorKey]}</section> : null}
    {customers.length === 0 ? <section className="rounded-card border border-[#f4c7ba] bg-[#fff5f2] p-5 text-sm leading-7 text-danger ring-1 ring-[#f4c7ba]">{en ? "There are no customers available for this booking. Create a customer first." : "Det finns inga kunder att koppla bokningen till. Skapa en kund först på kundsidan."}</section> : null}
    <form action={createBookingAction} className="grid gap-6 rounded-card border border-line bg-surface p-5 shadow-card sm:p-6">
      ...
      <section className="rounded-control border border-line bg-surface-subtle p-4 text-sm leading-7 text-ink-muted"><strong className="text-ink">{en ? "Controlled action:" : "Kontrollerad åtgärd:"}</strong> {en ? "The booking is saved manually and connected to the selected customer. No email is sent and no leads are changed." : "Bokningen sparas manuellt och kopplas till vald kund. Ingen e-post skickas och inga leads ändras."}</section>
      <button type="submit" disabled={customers.length === 0} className="inline-flex w-fit rounded-control bg-brand-deep px-5 py-3 text-sm font-semibold text-white transition hover:bg-brand-hover disabled:bg-[#9aa59d]">{en ? "Create booking" : "Skapa bokning"}</button>
    </form>
  </div>;
}
