...
export function WorkspaceMembersCard({ members, canManage }: { members: WorkspaceMember[]; canManage: boolean }) {
  return <article className="rounded-[24px] border border-[#e0e5dd] bg-white p-6 shadow-[0_14px_36px_rgba(20,43,32,0.045)]">
    ...
    {canManage ? <form action={addWorkspaceMemberAction} className="mt-5 grid gap-3 rounded-2xl bg-[#f7f9f6] p-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_160px_auto] sm:items-end">
      <label className="grid gap-1.5 text-xs font-semibold text-[#405047]">Namn (för inbjudan)<input name="name" maxLength={120} autoComplete="name" placeholder="För- och efternamn" className={field} /></label>
      ...
      <p className="text-xs leading-5 text-[#667168] sm:col-span-4">Finns kontot redan läggs användaren till direkt. Annars skickas en säker inbjudan som gäller i 48 timmar. Owner-rollen är skyddad.</p>
