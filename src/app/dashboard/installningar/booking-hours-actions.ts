...
function redirectWithError(error: BookingHoursError, formData: FormData): never { redirect(localized(`/dashboard/installningar?hours_error=${error}`, formData)); }
...
export async function updateWorkspaceBookingHoursAction(formData: FormData) {
  ...
  if (!access.ok || !canManageWorkspaceSettings(access)) redirectWithError("access", formData);
  const hours = bookingWeekdays.map(({ value }) => {
    ...
    if (!isClosed && (!isTime(opensAt) || !isTime(closesAt) || toMinutes(opensAt) >= toMinutes(closesAt))) redirectWithError("hours", formData);
  ...
  try {
    for (const hour of hours) {
      await sql`insert into workspace_booking_hours (workspace_id, weekday, opens_at, closes_at, is_closed) values (${access.workspaceId}, ${hour.weekday}, ${hour.opensAt}::time, ${hour.closesAt}::time, ${hour.isClosed}) on conflict (workspace_id, weekday) do update set opens_at = excluded.opens_at, closes_at = excluded.closes_at, is_closed = excluded.is_closed, updated_at = now()`;
    }
  } catch (error) {
    console.error("Failed to save workspace booking hours", error);
    redirectWithError("save", formData);
  }
  redirect(localized("/dashboard/installningar?hours_updated=1", formData));
}
