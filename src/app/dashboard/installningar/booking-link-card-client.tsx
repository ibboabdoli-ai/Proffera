...
type BookingLinkCardClientProps = {
  url: string;
  isPreview: boolean;
};
...
function resolveBookingUrl(url: string, isPreview: boolean) {
  if (typeof window === "undefined") return url;
  return resolveBookingUrlForLocation(url, isPreview, window.location.origin);
}

export function BookingLinkCardClient({ url, isPreview }: BookingLinkCardClientProps) {
  const resolvedUrl = useSyncExternalStore(
    subscribeToBrowserLocation,
    () => resolveBookingUrl(url, isPreview),
    () => url,
  );
