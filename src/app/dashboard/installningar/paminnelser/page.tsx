    ...
    <section className="grid gap-6 lg:grid-cols-[420px_1fr]">
      <form action={updateAction} className="h-fit rounded-[24px] border border-[#e0e5dd] bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 grid gap-4">
          ...
          <label className="grid gap-2 text-sm font-semibold">Timmar före bokningen<input name="hoursBefore" type="number" min="1" max="168" required defaultValue={settings.hoursBefore} className="rounded-xl border border-[#d9e1d7] px-4 py-3 font-normal" /><span className="text-xs font-normal text-[#6b766e]">1–168 timmar.</span></label>
      ...
      <article className="rounded-[24px] border border-[#e0e5dd] bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 grid gap-3">{deliveries.length ? deliveries.map((delivery) => <div key={delivery.id} className="rounded-xl border border-[#e3e8e1] bg-[#f8faf7] p-4 text-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold text-[#17201a]">{delivery.customerName} · {delivery.service}</p><p className="mt-1 text-xs text-[#68736b]">Bokning: {formatDate(delivery.startsAt)}</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-bold uppercase">{delivery.channel} · {delivery.status}</span></div>{delivery.errorMessage ? <p className="mt-3 text-xs font-semibold text-[#8f2f1b]">{delivery.errorMessage}</p> : null}<p className="mt-3 text-xs text-[#7a847d]">Planerad: {formatDate(delivery.scheduledFor)}</p></div>) : <p className="rounded-xl border border-dashed border-[#dce3da] p-5 text-sm text-[#7a847d]">Inga påminnelser har behandlats ännu.</p>}</div>
      </article>
    </section>
  </div>;
}
