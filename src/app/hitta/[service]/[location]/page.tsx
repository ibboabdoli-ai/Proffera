...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `Hitta verifierade företag för ${landing.serviceLabel.toLocaleLowerCase("sv-SE")} i ${landing.location}. Jämför företag och gå vidare till bokning, offert eller företagsprofil där det är tillgängligt.`;
  ...
  return (
    <main className="min-h-screen bg-canvas text-ink" lang="sv">
      <section className="border-b border-line bg-surface-subtle">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-muted sm:text-base">
            Hitta verifierade företag för {landing.serviceLabel.toLocaleLowerCase("sv-SE")} i {landing.location}. Endast publicerade företagsprofiler visas.
          </p>
      ...
      <div className="mx-auto max-w-7xl px-4 pb-12 pt-6 sm:px-6 lg:px-8">
        <PublicDirectoryResults locale="sv" search={search} />
      </div>
    </main>
  );
}
