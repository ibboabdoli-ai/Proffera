...
export default async function PrimeViewSeoPage({ params }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f6fb] text-[#09183a]">
      ...
      <header className="bg-[#06183b] text-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5"><Link href="/" className="text-xl font-black">PrimeView Window Care</Link><Link href="/#quote" className="rounded-xl bg-[#0a3c8f] px-4 py-3 text-sm font-black text-white">Free quote</Link></div></header>
      <section className="bg-[#06183b] px-5 pb-20 pt-14 text-white"><div className="mx-auto max-w-6xl"><p className="text-sm font-black uppercase tracking-[.16em] text-[#b8ceff]">PrimeView Window Care</p><h1 className="mt-4 max-w-4xl text-4xl font-black tracking-[-.04em] sm:text-6xl">{title}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-slate-200">{intro}</p><div className="mt-8 flex gap-3"><Link href="/#quote" className="rounded-xl bg-white px-5 py-4 font-black text-[#06183b]">Request a free quote</Link><a href={`tel:${primeViewSite.telephone}`} className="rounded-xl border border-white/40 px-5 py-4 font-black text-white">Call {primeViewSite.telephoneDisplay}</a></div></div></section>
