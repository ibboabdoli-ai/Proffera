...
const bookingErrors: Record<string, string> = {
  invalid: "Fyll i namn, e-post, tjänst och tid.",
...
async function requestPublicBooking(formData: FormData) {
  ...
  if (!sql || !slug || !name || !email || !serviceName || !startsAt || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) redirect(`/boka/${slug}?error=invalid`);
  ...
  const bookingHours = await sql`select opens_at::text as opens_at, closes_at::text as closes_at, is_closed from workspace_booking_hours where workspace_id = ${String(workspace.id)} and weekday = ${weekday} limit 1`;
  ...
  const validation = validatePublicBookingPolicy({
    ...
    bookingHour: bookingHours[0] ? { opensAt: String(bookingHours[0].opens_at), closesAt: String(bookingHours[0].closes_at), isClosed: Boolean(bookingHours[0].is_closed) } : null,
    timeZone,
  });
  ...
  const conflict = await sql`
    select id from bookings where workspace_id = ${String(workspace.id)} and status not in ('cancelled', 'no_show') and starts_at < ${end.toISOString()}::timestamptz and ends_at > ${start.toISOString()}::timestamptz
    union all
    select id from public_booking_verifications where workspace_id = ${String(workspace.id)}::uuid and consumed_at is null and expires_at > now() and starts_at < ${end.toISOString()}::timestamptz and ends_at > ${start.toISOString()}::timestamptz
    limit 1
  `;
  ...
  const result = await beginBookingEmailVerification({
    ...
    serviceName, city: String(workspace.primary_city ?? ""), startsAt: start.toISOString(), endsAt: end.toISOString(), timeZone,
  });
...
export default async function PublicBookingPage({ params, searchParams }: PageProps) {
  ...
  const timeZone = resolveBookingTimeZone(workspace.time_zone);
  const bookingForm = services.length && publishedHours.length ? <BookingRequestForm action={requestPublicBooking} slug={slug} services={services.map((service) => ({ name: String(service.name), durationMinutes: Number(service.duration_minutes) || 60, priceLabel: String(service.price_label ?? ""), bufferBeforeMinutes: Number(service.buffer_before_minutes) || 0, bufferAfterMinutes: Number(service.buffer_after_minutes) || 0, minimumNoticeMinutes: Number(service.minimum_notice_minutes) || 0, maximumAdvanceDays: Number(service.maximum_advance_days) || 365 }))} bookingHours={publishedHours.map((hour) => ({ weekday: Number(hour.weekday), opensAt: String(hour.opens_at).slice(0, 5), closesAt: String(hour.closes_at).slice(0, 5), isClosed: Boolean(hour.is_closed) }))} busyBookings={busyBookings.map((booking) => ({ startsAt: String(booking.starts_at), endsAt: String(booking.ends_at), bufferBeforeMinutes: Number(booking.buffer_before_minutes) || 0, bufferAfterMinutes: Number(booking.buffer_after_minutes) || 0 }))} timeZone={timeZone} variant={slug === "julius-salong" ? "salon" : "default"} /> : null;
  ...
  if (slug === "julius-salong") return <><JuliusBookingDemo live bookingContent={<div className="mt-6 rounded-[1.7rem] bg-white p-4 text-[#17201a] shadow-2xl lg:mt-0 lg:p-6"><p className="text-xs font-bold uppercase tracking-wide text-[#17452f]">Boka online</p><h2 className="mt-1 text-2xl font-black">Boka hos Elias</h2><p className="mt-4 rounded-2xl bg-[#e7f1eb] px-4 py-3 text-xs font-bold leading-5 text-[#17452f]">Efter att du valt tid skickar vi en sexsiffrig kod till din e-post. Tiden bokas först när koden är verifierad.</p>{notices}{bookingForm}</div>} />{aiChatClientId ? <BookingAiChatWidget clientId={aiChatClientId} /> : null}</>;
