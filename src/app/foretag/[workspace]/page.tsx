  ...
  return (
    <main style={style} className="min-h-screen px-4 py-6 sm:px-6 sm:py-10">
      ...
      <div className="mx-auto max-w-6xl">
          ...
          <div className={`grid ${experience.heroImageUrl || experience.heroVideoUrl ? "lg:grid-cols-[1.1fr_.9fr]" : ""}`}>
            <div className="p-7 sm:p-10 lg:p-12">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                ...
                {experience.contactEnabled ? <PublicBusinessTrackedLink workspaceId={business.id} eventKey="contact_clicked" href="#kontakt" className="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/35 px-5 font-black text-white">Kontakta oss</PublicBusinessTrackedLink> : null}
              </div>
            </div>
        ...
        {experience.contactEnabled ? <section id="kontakt" style={{ background: card }} className="mb-8 rounded-[2rem] p-7 ring-1 ring-black/10 sm:p-9"><div className="grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,.8fr)]"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--business-primary)]">Kontakt</p><h2 className="mt-2 text-3xl font-black">Redo att ta nästa steg?</h2><p style={{ color: muted }} className="mt-3 max-w-2xl text-sm leading-6">Skicka en förfrågan direkt till {business.companyName}. Den sparas i företagets Proffera-arbetsyta så att den kan följas upp.</p><div className="mt-6"><PublicBusinessContactForm workspaceId={business.id} /></div></div><aside className="grid content-start gap-3"><p className="text-sm font-black">Direktkontakt</p>{business.contactPhone ? <PublicBusinessTrackedLink workspaceId={business.id} eventKey="contact_clicked" href={`tel:${business.contactPhone}`} className="inline-flex min-h-12 items-center rounded-xl border border-black/10 px-5 font-bold"><Phone className="mr-2 h-4 w-4" /> {business.contactPhone}</PublicBusinessTrackedLink> : null}{business.contactEmail ? <PublicBusinessTrackedLink workspaceId={business.id} eventKey="contact_clicked" href={`mailto:${business.contactEmail}`} className="inline-flex min-h-12 items-center rounded-xl border border-black/10 px-5 font-bold"><Mail className="mr-2 h-4 w-4" /> {business.contactEmail}</PublicBusinessTrackedLink> : null}{bookingHref ? <PublicBusinessTrackedLink workspaceId={business.id} eventKey="book_clicked" href={bookingHref} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-[var(--business-primary)] px-5 font-black text-white">Boka online</PublicBusinessTrackedLink> : null}</aside></div></section> : null}
