  ...
  return (
    <main lang={locale} style={style} className="min-h-screen px-4 py-6 sm:px-6 sm:py-10">
      ...
      <div className="mx-auto max-w-5xl">
        ...
        {canQuote ? <section id="offert" style={{ background: card }} className="mt-6 rounded-[1.6rem] p-7 ring-1 ring-black/10 sm:p-8"><div className="mb-6"><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--business-primary)]">{serviceCopy.quoteEyebrow}</p><h2 className="mt-2 text-2xl font-black">{serviceCopy.quoteTitle}</h2><p style={{ color: muted }} className="mt-2 text-sm leading-6">{serviceCopy.quoteLead(business.companyName, item.name)}</p></div><PublicBusinessQuoteForm workspaceSlug={business.slug} serviceId={item.id} serviceName={item.name} locale={locale} /></section> : null}

        {canContact ? <section id="kontaktforfragan" style={{ background: card }} className="mt-6 rounded-[1.6rem] p-7 ring-1 ring-black/10 sm:p-8"><div className="mb-6"><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--business-primary)]">{serviceCopy.contactEyebrow}</p><h2 className="mt-2 text-2xl font-black">{serviceCopy.contactTitle}</h2><p style={{ color: muted }} className="mt-2 text-sm leading-6">{serviceCopy.contactLead(business.companyName)}</p></div><PublicBusinessContactForm workspaceId={business.id} serviceId={item.id} locale={locale} /></section> : null}
