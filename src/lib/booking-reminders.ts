...
type ReminderRow = { booking_id: string; workspace_id: string; workspace_name: string; customer_name: string; customer_email: string; customer_phone: string; service: string; city: string; starts_at: string; hours_before: number; email_enabled: boolean; sms_enabled: boolean };
  ...
  for (const raw of rows as unknown as ReminderRow[]) {
    ...
    for (const channel of channels) {
      ...
      const result = channel === "email"
        ? await sendBookingReminderEmail({ customerName: String(raw.customer_name), customerEmail: String(raw.customer_email), companyName: String(raw.workspace_name), service: String(raw.service), startsAt: new Date(String(raw.starts_at)).toISOString(), city: String(raw.city) })
        : await sendBookingReminderSms({ customerPhone: String(raw.customer_phone), companyName: String(raw.workspace_name), service: String(raw.service), startsAt: new Date(String(raw.starts_at)).toISOString() });
