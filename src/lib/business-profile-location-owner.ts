...
export const businessProfileLocationVisibilities = [
  "private",
  "approximate",
  "public",
] as const;
...
export type EditableBusinessProfileLocationPurpose =
  (typeof editableBusinessProfileLocationPurposes)[number];
export type BusinessProfileLocationVisibility =
  (typeof businessProfileLocationVisibilities)[number];
export type BusinessProfileLocationGeocodePrecision =
  (typeof businessProfileLocationGeocodePrecisions)[number];
...
function normalizeLocationId(value: unknown) {
  ...
  if (!uuidPattern.test(candidate)) throw new Error("Invalid Business Profile location id");
  return candidate;
}
...
export function normalizeBusinessProfileLocationWrite(input: WriteBusinessProfileLocationInput) {
  ...
  return {
    id: normalizeLocationId(input.id),
    purpose: input.purpose,
    visibility: input.visibility,
    isVisitable: Boolean(input.isVisitable),
    isPrimary: Boolean(input.isPrimary),
    confirmed: Boolean(input.confirmed),
    addressLine1: text(input.addressLine1, 250),
    postalCode: text(input.postalCode, 32),
    city: text(input.city, 120),
    municipality: text(input.municipality, 120),
    latitude,
    longitude,
    geocodeSource: text(input.geocodeSource, 80),
    geocodePrecision,
  };
}
...
function rowToLocation(row: Record<string, unknown>): DashboardBusinessProfileLocation {
  ...
  const visibility = businessProfileLocationVisibilities.includes(
    String(row.visibility) as BusinessProfileLocationVisibility,
  )
    ? String(row.visibility) as BusinessProfileLocationVisibility
    : "private";
...
export async function listOwnerBusinessProfileLocations(): Promise<DashboardBusinessProfileLocation[]> {
  ...
  const rows = await sql`
    select
      location.id::text,
      location.profile_id::text,
      location.purpose,
      location.visibility,
      location.is_visitable,
      location.is_primary,
    ...
    where profile.claimed_workspace_id = ${access.workspaceId}::uuid
      ...
      and (
        ...
        or location.owner_workspace_id = ${access.workspaceId}::uuid
      )
...
async function writeOwnerBusinessProfileLocation(input: WriteBusinessProfileLocationInput) {
  ...
  const rows = normalized.id
    ? await sql`
        with claimed_profile as (
          ...
          limit 1
          for update
        ), eligible_location as (
          select location.id
          from company_directory_profile_locations location
          join claimed_profile profile on profile.id = location.profile_id
          ...
          where ${normalized.isPrimary} = true
            and location.profile_id = (select id from claimed_profile)
            and location.id <> (select id from eligible_location)
        ...
        set purpose = ${normalized.purpose},
            ...
            updated_at = now()
        where location.id = (select id from eligible_location)
    ...
    : await sql`
        ...
        select
          ...
          ${normalized.geocodeSource},
          ${normalized.geocodePrecision},
          case when ${normalized.confirmed} then now() else null end
  ...
  if (!id) {
    throw new Error(normalized.id
      ? "Business Profile location is not owned by the currently claimed Workspace"
      : "The active Workspace does not own an eligible claimed Business Profile");
  }
  return { id };
}
...
export async function listAdminBusinessProfileLocations(profileId: string): Promise<DashboardBusinessProfileLocation[]> {
  ...
  const normalizedProfileId = normalizeLocationId(profileId);
...
export async function upsertAdminBusinessProfileLocation(
  profileId: string,
  input: WriteBusinessProfileLocationInput,
) {
  await requireSuperAdmin();
  const normalizedProfileId = normalizeLocationId(profileId);
  if (!normalizedProfileId) throw new Error("Business Profile id is required");
  ...
  const rows = normalized.id
    ? await sql`
        with eligible_profile as (
          select id from company_directory_profiles
          where id = ${normalizedProfileId}::uuid
            and is_active = true
          ...
          select location.id
          from company_directory_profile_locations location
          join eligible_profile profile on profile.id = location.profile_id
          where location.id = ${normalized.id}::uuid
            and location.source_type = 'admin'
            and location.is_active = true
          for update
        ), cleared_primary as (
          update company_directory_profile_locations location
          set is_primary = false, updated_at = now()
          where ${normalized.isPrimary} = true
            and location.profile_id = (select id from eligible_profile)
            and location.id <> (select id from eligible_location)
            and location.is_primary = true
            and location.is_active = true
          returning location.id
    ...
    : await sql`
        ...
        select
          ...
          ${normalized.geocodePrecision},
          case when ${normalized.confirmed} then now() else null end
        from eligible_profile profile
        returning id::text
      `;

  const id = String(rows[0]?.id ?? "");
  if (!id) throw new Error("Business Profile location is not admin-editable");
  return { id };
}
