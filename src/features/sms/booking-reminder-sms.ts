type Input = { customerPhone: string; companyName: string; service: string; startsAt: string };
...
function formatDate(value: string) {
  return new Intl.DateTimeFormat("sv-SE", { timeZone: "Europe/Stockholm", weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(value));
}
...
export async function sendBookingReminderSms(input: Input) {
  ...
  const content = `Påminnelse: du har en bokning hos ${input.companyName}, ${input.service}, ${formatDate(input.startsAt)}.`;
  try {
