type Input = { customerName: string; customerEmail: string; companyName: string; service: string; startsAt: string; city: string };
...
function formatDate(value: string) {
  return new Intl.DateTimeFormat("sv-SE", { timeZone: "Europe/Stockholm", dateStyle: "full", timeStyle: "short" }).format(new Date(value));
}
...
export async function sendBookingReminderEmail(input: Input) {
  ...
  const when = formatDate(input.startsAt);
