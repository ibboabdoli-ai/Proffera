...
alter table workspace_services drop constraint if exists workspace_services_public_status_check;
alter table workspace_services add constraint workspace_services_public_status_check check (public_status in ('draft', 'published', 'hidden')) not valid;
alter table workspace_services validate constraint workspace_services_public_status_check;

alter table workspace_services drop constraint if exists workspace_services_conversion_mode_check;
alter table workspace_services add constraint workspace_services_conversion_mode_check check (conversion_mode in ('book', 'quote', 'book_or_quote', 'contact')) not valid;
alter table workspace_services validate constraint workspace_services_conversion_mode_check;

alter table workspace_services drop constraint if exists workspace_services_public_slug_check;
alter table workspace_services add constraint workspace_services_public_slug_check
  check (public_slug is null or (char_length(public_slug) between 2 and 120 and public_slug ~ '^[a-z0-9]+(?:-[a-z0-9]+)*$')) not valid;
alter table workspace_services validate constraint workspace_services_public_slug_check;

alter table workspace_services drop constraint if exists workspace_services_public_copy_check;
...
create unique index if not exists workspace_services_workspace_public_slug_unique_idx
  on workspace_services (workspace_id, public_slug) where public_slug is not null;
...
alter table bookings drop constraint if exists bookings_service_ws_fk;
alter table bookings add constraint bookings_service_ws_fk
...
create index if not exists bookings_workspace_service_idx
  on bookings (workspace_id, service_id, starts_at desc) where service_id is not null;

alter table public_booking_verifications add column if not exists service_id uuid;
...
alter table public_booking_verifications drop constraint if exists public_booking_verifications_service_fk;
alter table public_booking_verifications add constraint public_booking_verifications_service_fk
  foreign key (service_id) references workspace_services (id) on delete set null not valid;
...
create or replace function enforce_uuid_workspace_service_identity()
returns trigger
language plpgsql
...
begin
  if new.service_id is not null and not exists (
    ...
    raise exception 'service_id does not belong to workspace' using errcode = '23503';
  end if;
  return new;
end;
$$;
...
before insert or update of workspace_id, service_id
on public_booking_verifications
for each row execute function enforce_uuid_workspace_service_identity();

alter table workspace_service_jobs add column if not exists service_id uuid;
...
alter table workspace_service_jobs drop constraint if exists workspace_service_jobs_service_fk;
alter table workspace_service_jobs add constraint workspace_service_jobs_service_fk
  foreign key (service_id) references workspace_services (id) on delete set null not valid;
...
create index if not exists workspace_service_jobs_workspace_service_idx
  on workspace_service_jobs (workspace_id, service_id, created_at desc) where service_id is not null;
...
begin
  if new.service_id is null and new.booking_id is not null then
    ...
    from bookings booking
    where booking.id = new.booking_id and booking.workspace_id = new.workspace_id::text;
  end if;

  if new.service_id is null and new.quote_request_id is not null then
    select request.service_id into new.service_id
    from workspace_quote_requests request
    where request.id = new.quote_request_id and request.workspace_id = new.workspace_id;
  end if;

  if new.service_id is null and nullif(trim(new.service_name), '') is not null then
    select service.id into new.service_id
    from workspace_services service
    where service.workspace_id = new.workspace_id::text and service.name = new.service_name
    limit 1;
  end if;

  if new.service_id is not null and not exists (
    select 1 from workspace_services service
    where service.id = new.service_id and service.workspace_id = new.workspace_id::text
  ) then
...
alter table workspace_experience_settings drop constraint if exists workspace_experience_public_home_mode_check;
alter table workspace_experience_settings add constraint workspace_experience_public_home_mode_check check (public_home_mode in ('booking', 'website')) not valid;
alter table workspace_experience_settings validate constraint workspace_experience_public_home_mode_check;

alter table workspace_experience_settings drop constraint if exists workspace_experience_business_intro_check;
alter table workspace_experience_settings add constraint workspace_experience_business_intro_check check (char_length(business_intro) <= 2000) not valid;
...
create table if not exists public_business_events (
  ...
  constraint public_business_events_text_check
    check (char_length(path) <= 500 and char_length(session_key) <= 120 and char_length(referrer) <= 1000)
);
...
create index if not exists public_business_events_workspace_service_created_idx
  on public_business_events (workspace_id, service_id, created_at desc) where service_id is not null;

drop trigger if exists public_business_events_service_identity on public_business_events;
create trigger public_business_events_service_identity
before insert or update of workspace_id, service_id
on public_business_events
for each row execute function enforce_uuid_workspace_service_identity();
...
comment on column workspace_experience_settings.public_home_mode is 'Custom-domain root mode. Existing workspaces stay booking-first by default.';
comment on table public_business_events is 'Minimal first-party funnel telemetry for the public business hub. No customer PII is stored.';
