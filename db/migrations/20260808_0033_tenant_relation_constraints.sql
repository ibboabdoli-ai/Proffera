...
--
-- The application already scopes reads/writes by Workspace. These composite
-- foreign keys additionally prevent cross-Workspace references at the database
-- layer even while the runtime still connects through a role that can bypass
-- row-level security.
--
...
-- types already match. Type normalization and RLS are intentionally separate
...
-- TEXT workspace_id relations.
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'booking_reminders_booking_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'bookings_customer_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'bookings_staff_ws_fk') then
    alter table bookings
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'customer_events_customer_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'customer_events_booking_ws_fk') then
    alter table customer_events
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'staff_schedules_staff_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'staff_time_off_staff_ws_fk') then
...
-- UUID workspace_id relations.
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'quote_offers_request_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'service_jobs_quote_request_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'service_jobs_quote_offer_ws_fk') then
    alter table workspace_service_jobs
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'job_attachments_job_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'job_events_job_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'job_notes_job_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'job_payments_job_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'job_evidence_job_ws_fk') then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'job_evidence_attachment_ws_fk') then
    alter table workspace_service_job_completion_evidence
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'website_reviews_invitation_ws_fk') then
