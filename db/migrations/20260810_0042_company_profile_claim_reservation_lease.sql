...
alter table company_directory_profiles
  add column if not exists claim_reservation_token uuid;
...
alter table company_directory_profiles
  ...
  check (
    (
      claim_reservation_id is null
      and claim_reserved_at is null
      and claim_reservation_token is null
    )
    or (
      claim_reservation_id is not null
      and claim_reserved_at is not null
      and claim_reservation_token is not null
    )
  );
