...
`,
  ...
  it("allows a second Phase-1 writable Worker but blocks a third", () => {
    const phase1 = (taskId: string, graph: string, path: string, number: number) =>
      workerPr({
        number,
        ...
        body: [
          `Task ID: ${taskId}`,
          `Graph path: ${graph}`,
          `Allowed paths: [${JSON.stringify(path)}]`,
          packetComment(packet({
            task_id: taskId,
            graph_path: graph,
            branch: `work/proffera-${taskId.toLowerCase()}`,
            allowed_paths: [path],
            forbidden_paths: ["src/never/"],
          })),
        ].join("\n"),
        files: [`${path}a.ts`],
      });

    const first = phase1("OTHER-1", "feature/one", "src/features/one/", 901);
    expect(evaluate(baseContext({ open_prs: [first] })).status).toBe("TASK_CREATED");
