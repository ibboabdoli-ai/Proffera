...
describe("Company Directory service-city SEO landings", () => {
  ...
  it("requires three safe published profiles with one complete SCB workplace", () => {
    ...
    expect(helper).toContain("profile.claimed_workspace_id is null");
    expect(helper).toContain("jsonb_array_length(coalesce(scb.workplaces, '[]'::jsonb)) = 1");
    expect(helper).toContain("{visitingAddress,city}");
    expect(helper).not.toContain("profile.municipality");
