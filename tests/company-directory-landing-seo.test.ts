...
describe("Company Directory service-city SEO landings", () => {
  ...
  it("requires a useful published-result floor before a landing becomes indexable", () => {
    ...
    expect(helper).toContain("having count(distinct profile.id) >= ${DIRECTORY_LANDING_MIN_BUSINESSES}");
